		     											    		...
		     											    		success: function (res) {
				     													...
				     													jobTab.after(
				     															...
				     															.attr("data-id", res.jobId)
				     								     						.addClass("job-input flex flex-col p-16 line-base border-1 round-6 mb-16")
				     								     						.append($("<input>")
				     								     									.attr("placeholder", "제목")
				     								     						...
				     								     						.append($("<div>")
				     								     									.addClass("job-datetime").append(
																			    	    				$("<span>").text("날짜 입력").addClass("text-12 text2-font-color")
																	    	    				).append(
																	    	    					$("<div>")
																	    	    						.append($("<input>").attr("value", calcDate(res.jobStartTime)).attr("type", "datetime-local").addClass("start-time-input mr-8"))
																	    	    						.append($("<span>").text("~").addClass("mr-8"))
																	    	    						.append($("<input>").attr("value", calcDate(res.jobEndTime)).attr("type", "datetime-local").addClass("end-time-input"))
																	    	    						.addClass("mt-8")))
				     								     						.append($("<div>")
				     								     						...
				     								     						.append($("<div>")
				     																...
				     																if (res.jobPlace != null) {
				     																	$('[data-id="' + res.jobId + '"] .job-place').append(
		     										     											$("<span>")
					     										     									.text("장소")
					     										     									.addClass("text-12 subtitle-font-color"))
			     										     									.append(
			     										     											$("<input>")
					     										     									.val(res.jobPlace)
					     										     									.addClass("job-place-address border-1 line-base outline-none round-6 px-8 py-4 text-12 text-font-color mt-8"))
			     										     									.append(
			     										     											$("<div>")
					     										     									.addClass("map mt-8 round-6"))
					     										     							.addClass("mt-8")
					     										     									
					     										     									var container = $('[data-id="' + res.jobId + '"].job-input .map')[0];
    		    		...
    		    		if (val.jobPlace != null) {
    						...
    						geocoder.addressSearch(address, function (res, status) {
    							if (status === kakao.maps.services.Status.OK) {
    								...
    								$('[data-id="' + val.jobId + '"] .job-place-tab').remove();
    							}
    						})
    		    		}
    		    		...
    		    		if (val.jobDescription != null) {
    		    			var jobDescription = $("<p>")
								.text(val.jobDescription)
									.addClass("text-14 text-font-color mt-8");
    		    			$('[data-id="' + val.jobId + '"] .job-description-tab').append(jobDescription);
    		    		}
    		    })
    		  },
    	  })
